...
class _RegularDepositsFormsState extends State<RegularDepositsForms> {
  ...
  void _updateInputsFromState(FinancialCalculation deposit) {
    final formatter = DecimalInputFormatter();

    _depositAmountController.value = deposit.periods > 0
        ? formatter.formatEditUpdate(
            TextEditingValue.empty,
            TextEditingValue(
              text: deposit.periods.toStringAsFixed(2).replaceAll('.', ''),
            ),
          )
        : TextEditingValue.empty;

    _monthsController.text = deposit.periods > 0 ? deposit.periods.toString() : '';
  ...
  FinancialCalculation _getRegularDepositFromInputs() {
    return FinancialCalculation(
      initialValue: _parseFormattedDouble(_depositAmountController.text),
